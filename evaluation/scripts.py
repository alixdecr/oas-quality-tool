...
# ----------------------------------
# QUALITY SCATTER PLOT BY DIMENSION
# ----------------------------------
for dimension in ["format", "oas-version", "metadata", "server", "descriptions", "examples"]:
    ...
    for json_path in OUTPUTS_PATH.glob("*.json"):
        ...
        nb_routes = data["structure"]["routes"]["total"]
        quality = data["evaluation-groups"][dimension]["pass"] / data["evaluation-groups"][dimension]["total"]
        color = "tab:blue"
    ...
    plt.xscale("log")
    plt.xlabel("API Size", fontweight="bold", fontsize=16, labelpad=20)
    plt.ylabel(f"Quality of {dimension}", fontweight="bold", fontsize=16, labelpad=20)
